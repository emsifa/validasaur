...
export const assertInvalid = (result: RuleReturn, payload: InvalidPayload, message?: string): void => {
  ...
  if (result !== undefined) {
    ...
    assertEquals(result.implicit, payload.implicit, `Result implicity should be: ${result.implicit ? 'true' : 'false'}`);
  }
}
