...
Deno.test("rules.dateBetween() date check", () => {
  ...
  assertValid(
    dateBetween(minDate, maxDate)(new Date("2020-01-02"), fakeUtils) as Validity
  );
  // date max
  assertValid(
  ...
  assertValid(
    dateBetween(minDate, maxDate)(new Date("2020-01-09"), fakeUtils) as Validity
  );
  ...
  assertValid(
    dateBetween(minDate, maxDate)(new Date("2020-01-07"), fakeUtils) as Validity
  );
});
