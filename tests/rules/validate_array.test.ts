...
import { Validity, Rule } from "../../src/types.ts";
import { isNumber } from "../../src/rules/is_number.ts";
import { assertEquals, assertNotEquals } from "../deps.ts";
import { required } from "../../src/rules/required.ts";

Deno.test("rules.validateArray(true, [isNumber]) should return required rule first", () => {
  const rules = validateArray(true, [isNumber]);
  assertEquals(rules[0], required);
});

Deno.test("rules.validateArray(false, [isNumber]) shouldn't return required rule first", () => {
  const rules = validateArray(false, [isNumber]);
  assertNotEquals(rules[0], required);
});

Deno.test("rules.validateArray(false, [isNumber])[0](null) should be valid", async () => {
...
Deno.test("rules.validateArray(true, [isNumber])[1]([]) should be valid", async () => {
...
Deno.test("rules.validateArray(true, [isNumber], { minLength: 1 })[1]([]) should be invalid", async () => {
...
Deno.test("rules.validateArray(true, [isNumber], { minLength: 1 })[1]([100]) should be valid", async () => {
  const rule = validateArray(true, [isNumber], { minLength: 1 })[1] as Rule;
  assertNotEquals(rule, required);
  assertEquals(typeof rule, "function");
...
Deno.test("rules.validateArray(true, [isNumber], { maxLength: 3 })[1]([1, 2, 3]) should be valid", async () => {
...
Deno.test("rules.validateArray(true, [isNumber], { maxLength: 3 })[1]([1, 2, 3, 4]) should be invalid", async () => {
  const rule = validateArray(true, [isNumber], { maxLength: 3 })[1] as Rule;
  assertNotEquals(rule, required);
  assertEquals(typeof rule, "function");
